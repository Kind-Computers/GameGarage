...
$sourceFiles = @($manifest | Where-Object { $_ -match '^(GameGarage/|tests/|benchmarks/|scripts/build\.ps1$|global\.json$|Directory\.Build\.)' } | Sort-Object)
...
if ($Tag -ne "v$($record.version)" -or $Tag -ne 'v0.1.0-preview.1') { throw 'Tag does not match the prepared preview version.' }
...
foreach ($check in @('windows11EnglishSmoke', 'cleanMachineWithoutDotNet', 'keyboardAndDisplayScaling', 'repairConfirmationsAndDisposableMaintenance', 'packageAndSourceAudit')) {
    if ($record.$check -isnot [bool] -or $record.$check -ne $true) { throw "Release acceptance remains incomplete: $check" }
}
